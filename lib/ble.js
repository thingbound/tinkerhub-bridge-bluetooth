...
const { Appliance, Battery, State, Storage } = require('appliances');

const SERVICE_BATTERY = '180f';
const C_BATTERY_LEVEL = '2a19';

module.exports = class BLE extends Appliance.with(State, Storage) {
	static get type() {
	...
	init() {
		return super.init()
			.then(() => {
				this.storage.get('shouldConnect')
					.then(l => this.setShouldConnect(l || false));
			})
			.then(() => this);
	}
	...
	bleConnect() {
		return this.storage.set('shouldConnect', true)
			.then(() => this.setShouldConnect(true));
	}

	bleDisconnect() {
		return this.storage.set('shouldConnect', false)
			.then(() => this.peripheral.disconnect())
			.then(() => true);
	}
